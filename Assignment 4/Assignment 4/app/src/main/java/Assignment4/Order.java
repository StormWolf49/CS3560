...
public class Order {
	...
	public void createOrder() throws ClassNotFoundException, SQLException {
		...
		PreparedStatement stmt = connection
				.prepareStatement("INSERT INTO order(number, date, item, price) VALUES (?,?,?,?)");
		stmt.setInt(1, this.getNumber());
		stmt.setDate(2, (java.sql.Date) this.getDate());
		stmt.setString(3, this.getItem());
		stmt.setDouble(4, this.getPrice());
		stmt.executeUpdate();

		stmt = connection.prepareStatement("SELECT * FROM order by number desc");
